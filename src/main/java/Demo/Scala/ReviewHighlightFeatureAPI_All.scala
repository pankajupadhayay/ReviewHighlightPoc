...
object ReviewHighlightFeatureAPI_All extends App {
  ...
  val namesDF_feature = spark.sql("select product_key_feature_id,product_external_id,feature,feature_type,total_reviews_mentioned,(positive_reviews_mentioned+negative_reviews_mentioned) as mentionsCount  FROM translatedProductSummaryKeyFeatures  where product_external_id='005010115'")
  ...
  case class ProductFeatures(var product_key_feature_id: String, var product_external_id: String, var feature: String, var feature_type: String, var total_reviews_mentioned: Int,
  ...
  case class ProductSummaryFeaturesList(fit:ProductSummaryFeatures,satisfaction:ProductSummaryFeatures,quality:ProductSummaryFeatures,comfort:ProductSummaryFeatures,price:ProductSummaryFeatures)
  val productFeatureBest=ProductSummaryFeaturesList(aMapFeatureBest.get(arrayArrayFeatureBest.lift(0).get).get,aMapFeatureBest.get(arrayArrayFeatureBest.lift(1).get).get,aMapFeatureBest.get(arrayArrayFeatureBest.lift(2).get).get,aMapFeatureBest.get(arrayArrayFeatureBest.lift(3).get).get,aMapFeatureBest.get(arrayArrayFeatureBest.lift(4).get).get)
  ...
  val productFeatureWorse=ProductSummaryWorseFeaturesList(aMapFeatureWorse.get(arrayArrayFeatureWorse.lift(0).get).get,aMapFeatureWorse.get(arrayArrayFeatureWorse.lift(1).get).get,aMapFeatureWorse.get(arrayArrayFeatureWorse.lift(2).get).get,aMapFeatureWorse.get(arrayArrayFeatureWorse.lift(3).get).get)
  ...
  // PrintWriter
  import java.io._
  val pw = new PrintWriter(new File("/Users/pankaj.upadhayay/Documents/Dev/SampleData/ReviewHighlight_Levis.json" ))
  pw.write(json)
  pw.close
